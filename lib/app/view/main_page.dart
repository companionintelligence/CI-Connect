...
class MainPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          BlocSelector<AppBloc, AppState, AppAuthenticated?>(
            selector: (state) => state is AppAuthenticated ? state : null,
      ...
      body: BlocSelector<AppBloc, AppState, AppAuthenticated?>(
        selector: (state) => state is AppAuthenticated ? state : null,
        builder: (context, authenticatedState) {
          if (authenticatedState != null) {
  ...
  Widget _buildAuthenticatedContent(BuildContext context, AuthSession session) {
    return BlocSelector<AppBloc, AppState, AuthSession>(
      selector: (state) => state is AppAuthenticated ? state.session : session,
                ...
                child: Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      _buildInfoRow('Name', currentSession.user.name),
                      _buildInfoRow('Email', currentSession.user.email),
                      _buildInfoRow('User ID', currentSession.user.id),
                    ],
                  ),
                ),
              ),
                ...
                child: Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      _buildInfoRow(
                        'Session Token',
                        '${currentSession.sessionToken.substring(0, 20)}...',
                      ),
                      _buildInfoRow(
                        'Access Token',
                        '${currentSession.accessToken.substring(0, 20)}...',
                      ),
                      if (currentSession.createdAt != null)
                        _buildInfoRow(
                          'Session Created',
                          _formatDateTime(currentSession.createdAt!),
                        ),
                    ],
                  ),
                ),
